...
fn get_cmd(tmp_type: VideoContainerType, input_file: &str, output_file: &str, quality_val: i32) -> String {
    match tmp_type {
        VideoContainerType::M3u8 => {
            format!("{}/resource/ffmpeg.exe -i {} -f hls {}", now_dir_path(),
                    input_file, output_file)
        }

        _ => {
            format!("{}/resource/ffmpeg.exe -i {} -b {}k {}", now_dir_path(),
                    input_file, quality_val, output_file)
        }
    }
}
