...
use iced::{
    Align, Button, Column, Command, Container, Length, PickList, Row, Scrollable, Slider, Text,
};
use iced_style::{button_style, container_style, pick_list_style, scrollable_style};
            ...
            Column::new()
                ...
                .push(
                    Row::new()
                        ...
                        .align_items(Align::Center)
                        .push(
                            Button::new(&mut home_state.file_home_btn, Text::new("选择文件"))
                                .padding(5)
                                .style(button_style::Button::Primary)
                                .on_press(Message::FileSelected),
                        )
                        .push(Text::new(&home_state.create_video_path).size(18)),
                ),
        )
        .push(
            Container::new(
...
pub fn get_command(
    ...
    for file in file_list {
        ...
        let create_path = get_create_path(
            t_path.clone(),
            home_state.select_video_type,
            filename.to_string(),
        );
        com_arr.push(Command::perform(
            formatting_video(tmp_type, file.clone(), create_path, index, quality_val),
            Message::ReceiveMsg,
        ));
        index += 1;

        let old_msg = &home_state.msg_conversion_statue;
        home_state.msg_conversion_statue = format!(
            "{}{}   转换中...\r\n\
                    ",
            old_msg, filename
        );
